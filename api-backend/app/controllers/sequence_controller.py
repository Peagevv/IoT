...
class SequenceController:
    ...
    @staticmethod
    def get_sequence_operations_detailed(id_secuencia):
        """Obtener operaciones detalladas de una secuencia con toda la info necesaria"""
        try:
            sequence = SequenceModel.get_sequence_by_id(id_secuencia)
            ...
            # Las operaciones ya vienen con toda la info del JOIN
            return make_response(jsonify({
                'status': 'success',
                'data': {
                    'id_secuencia': sequence['id_secuencia'],
                    'nombre_secuencia': sequence['nombre_secuencia'],
                    'operaciones': sequence['operaciones'],  # Ya tiene id, nombre, duración, etc.
                    'total_operaciones': len(sequence['operaciones'])
                }
            }), 200)
